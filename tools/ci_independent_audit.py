...
EXCLUDE_DIRS = {".git", "__pycache__", ".pytest_cache", "node_modules", "target", "archive"}
...
def audit_metadata(findings: list[str]) -> None:
    ...
    if bound != REQUIRED_STANDARDS:
        fail(findings, "F-ATCLANG-STD-001 P1 governance: .atc/standards.yaml required_standards drift")
    ...
    if 'license = { text = "All Rights Reserved' in pyproject:
        fail(findings, "F-ATCLANG-LIC-001 P1 consistency: pyproject conflicts with repository Apache-2.0 license")
    ...
    if "Rust is canonical" not in readme or "Python" not in readme:
        fail(findings, "F-ATCLANG-ARCH-001 P1: README does not explicitly enforce Rust-production/Python-reference boundary")
...
def audit_workflows(findings: list[str]) -> None:
    ...
    for path in sorted(workflows.glob("*.yml")) + sorted(workflows.glob("*.yaml")):
        ...
        if "pull_request_target" in text:
            fail(findings, f"F-ATCLANG-CI-001 P1 security: pull_request_target in {path.relative_to(ROOT)}")
        if re.search(r"actions/checkout@(main|master)\b", text):
            fail(findings, f"F-ATCLANG-CI-002 P1 supply-chain: mutable checkout ref in {path.relative_to(ROOT)}")
        for line_no, line in enumerate(text.splitlines(), 1):
            if "uses:" in line and re.search(r"uses:\s*[^@]+@(v|main|master|dev|latest)\b", line):
                fail(findings, f"F-ATCLANG-CI-003 P1 supply-chain: unpinned action {path.relative_to(ROOT)}:{line_no}")
...
def audit_security_and_determinism(findings: list[str]) -> None:
    ...
    if 'return isinstance(sig, str) and sig.startswith("sig_")' in vm:
        fail(findings, "F-20260916-ATCLANG-001 P1 crypto: reference ECDSA verifier still accepts arbitrary sig_* values")
    if "return bool(token) and len(token) > 10" in vm:
        fail(findings, "F-20260916-ATCLANG-002 P1 auth: reference JWT verifier still accepts arbitrary long tokens")
    if "def net_send" in vm and "return True  # Simulation" in vm:
        fail(findings, "F-20260916-ATCLANG-003 P1 network: reference net_send reports success without transport")
    ...
    if "words = [" in vm and "generate_atc_address" in vm and "secrets.token_bytes(32)" in vm:
        fail(findings, "F-20260916-ATCLANG-005 P1 wallet: reference mnemonic/address implementation is not protocol-conformant")
    if "time.time()" in vm or "secrets.token_bytes" in vm or "secrets.randbits" in vm:
        fail(findings, "F-20260916-ATCLANG-006 P2 determinism: Python reference VM contains nondeterministic host operations; Rust consensus path must reject them")
...
def audit_placeholders_and_docs(findings: list[str]) -> None:
    patterns = re.compile(r"\b(TODO|FIXME|HACK|XXX|unimplemented!|NotImplementedError|STUB|placeholder)\b", re.I)
    ...
    for path in files():
        ...
        if patterns.search(text):
            fail(findings, f"F-ATCLANG-COMP-001 P2 completeness: unresolved placeholder/stub marker in {rel}")

    roadmap = read(ROOT / "ROADMAP.md")
    if "G3" not in roadmap or "G18" not in roadmap or "G19" not in roadmap:
        fail(findings, "F-ATCLANG-DOC-001 P2 roadmap: mandatory compiler/security/release gates are not represented")
    ...
    if "F-20260916-ATCLANG-001" not in audit or "F-20260916-ATCLANG-007" not in audit:
        fail(findings, "F-ATCLANG-DOC-002 P1 traceability: audit record does not contain all known P1/P2 findings")
...
def audit_evidence(findings: list[str]) -> None:
    ...
    if expected and (not bound or bound.group(1) != expected):
        fail(findings, "F-ATCLANG-EVD-001 P1 evidence: evidence.yaml is not bound to the current CI commit")
    if expected and status and status.group(1) in {"pass", "pass_with_evidence"} and (not bound or bound.group(1) != expected):
...
def audit_inventory(findings: list[str]) -> None:
    ...
    if missing or stale:
        ...
        fail(findings, f"F-ATCLANG-FILE-001 P1 inventory: FILE_REGISTER.md drift; missing={len(missing)} sample={sample!r}, stale={len(stale)}")
...
def main() -> int:
    ...
    print("")
    if findings:
        print("RESULT: FAIL — release remains blocked until findings are remediated and re-verified.")
        return 1
    print("RESULT: PASS — static/governance audit passed. Runtime/CI evidence remains required for release readiness.")
    return 0
